 ...
 */
public class LR1SetGeneratorReference {

    // Grammar.
    private AbstractGrammar grammar;

    // Complete state space.
    public HashSet<LR1Set> states;
     ...
     *            Grammar.
     */
    public LR1SetGeneratorReference(AbstractGrammar grammar) {
    ...
    public boolean hasConflicts(LR1Set set) {
        Set<Alphabet> lookaheads = new HashSet<>();
        for(LR1Item item : set){
            lookaheads.add(item.getLookahead());
        }
        for(Alphabet lo : lookaheads){
            if(hasConflicts(set, lo)){
                return true;
            }
        }
        return false;
    ...
    public boolean hasConflicts(LR1Set set, Alphabet lookahead) {
        int canShift = 0;
        int canReduce = 0;

        for (LR1Item item : set) {
            if (item.canShiftOverTerminal(lookahead))
                canShift++;
