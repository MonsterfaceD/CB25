 ...
 */
public class LR0SetGeneratorReference {

	// Grammar.
	private AbstractGrammar grammar;

	// Complete state space.
	public HashSet<LR0Set> states;
	 ...
	 *            Grammar.
	 */
	public LR0SetGeneratorReference(AbstractGrammar grammar) {
	...
	private void generateLR0StateSpace() {
		...
		while (!queue.isEmpty()) {
			...
			for (Alphabet symbol : set.getShiftableSymbols()) {
				LR0Set newSet = new LR0Set(set.getSequence(), symbol);
	...
	private LR0Set epsilonClosure(LR0Set set) {
		LR0Set result = new LR0Set(set.getSequence());
	...
	public int nrConflicts() {
		...
		for (LR0Set lr0set : states) {
			if (hasConflicts(lr0set))
				counter++;
		}
		return counter;
	}
	
	public boolean hasConflicts(LR0Set set) {
		int canShift = 0;
		int canReduce = 0;
