...
tests_ex6 = [
    ("generator/valid/", "gcd1", "GCD: \n3\n"),
    ("generator/valid/", "gcd2", "GCD:\n13\n"),
    ("generator/valid/", "gcd3", "GCD:\n720\n"),
    ("generator/valid/", "factorial1", "Factorial:\n720\n"),
    ("generator/valid/", "factorial2", "1\n2\n6\n24\n120\n720\n5040\n40320\n362880\n3628800\n39916800\n479001600\n"),
    ("generator/valid/", "generatorTest1", "Hello world!\n16\n5\n"),
    ("generator/valid/", "generatorTest2", "not here\n"),
    ("generator/valid/", "evenodd1", "odd\nodd\n"),
    ("generator/valid/", "evenodd2", "even\neven\n"),
    ("generator/valid/", "collatz1", "24\n12\n6\n3\n10\n5\n16\n8\n4\n2\n1\n"),
    ("generator/valid/", "collatz2", "25\n76\n38\n19\n58\n29\n88\n44\n22\n11\n34\n17\n52\n26\n13\n40\n20\n10\n5\n16\n8\n4\n2\n1\n"),
    ("generator/valid/", "collatz3", "111\n"),
    ("generator/valid/", "fahrenheit", "Please enter a temperature in Celsius without comma/point: (example: 32.15 should be entered as 3215)\nTemperature in Fahrenheit is: \n4919900\n"),
	("generator/valid/", "fizzbuzz", "Fizz\nBuzz\n1\n2\nFizz\n4\nBuzz\nFizz\n7\n8\nFizz\nBuzz\n11\n"),
	("generator/valid/", "nonono", "A1\nB1\n"),
	("generator/valid/", "quotient_and_remainder", "Enter dividend: \nEnter divisor: \nQuotient = \n39317\n and Remainder = \n140\n"),
	("generator/valid/", "random_walk", "I stopped walking after: \n10\n steps\n"),
	("generator/valid/", "variables_and_write", "a string\n123456\n0\n"),
]
...
def call_process(args, test_file):
    ...
    """
    try:
        #print("Process call: {}".format(' '.join(args))) # DEBUG ONLY
...
def check_ex5(bin_path, name):
    ...
    return correct == len(tests_ex5)

def check_ex6(bin_path, name, quiet):
    correct = 0
    tests_orig_path = os.path.join(exercise_path, "i2CompilerSolution/tests")
    for (test_path, test_file, result) in tests_ex6:
        test_sol_path = os.path.join(bin_path, "..", "tests", test_path)
        # Create test dir
        if not os.path.exists(test_sol_path):
            os.makedirs(test_sol_path)
        # Copy test file
        shutil.copyfile(os.path.join(tests_orig_path, test_path, "{}.txt".format(test_file)), os.path.join(test_sol_path, "{}.txt".format(test_file)))

        # Generate Jasmin code
        if not quiet:
            print("Generating Jasmin code for {}".format(test_file))
        args = ['java', "-cp",  bin_path, "Main", os.path.join(test_sol_path, "{}.txt".format(test_file)), "--out", os.path.join(test_sol_path, "{}.j".format(test_file))]
        output = call_process(args, "{} (generating Jasmin code)".format(test_file))
        if output is None:
            continue

        # Generate Java file
        if not quiet:
            print("Generating Java file")
        args = ['java', "-jar", os.path.join(bin_path, "..", "lib", "jasmin.jar"), "-d", test_sol_path, "-g", os.path.join(test_sol_path, "{}.j".format(test_file))]
        output = call_process(args, "{} (generating java file)".format(test_file))
        if output is None:
            continue
        elif "JAS Error" in output:
            print("Failed to translate generated Jasmin code, Jasmin tool reported: {}".format(output))
            continue
        elif re.search(r"Found (\d+) errors", output, re.IGNORECASE | re.MULTILINE):
            print("Failed to translate generated Jasmin code, Jasmin tool reported: {}".format(output))
            continue
        ...
        args = ["java", "-cp", test_sol_path, test_file]
        output = call_process(args, "{} (running program)".format(test_file))
        if output is None:
            continue
        if output == result:
            correct += 1
        else:
            print("Incorrect result on {}: {} but expected {}".format(test_file, output, result))
            print("Correct result (hex): {}. Student solution (hex): {}".format(''.join(x.encode('hex') for x in result), ''.join(x.encode('hex') for x in output)))
    print("{} of {} correct".format(correct, len(tests_ex6)))
    return correct == len(tests_ex6)
...
def check_one(path, name, number, quiet):
    ...
    if number == 1:
        ...
        print("Checking exercise 6 for {}".format(name))
        return check_ex6(bin_path, name, quiet)
    else:
