...
public class RegexGrammarRunner {
	...
	public static Automaton getAutomaton(CharStream regex, Boolean printRegex) {
		...
		RegexGrammarLexer lexer = new RegexGrammarLexer(regex);
		CommonTokenStream tokens = new CommonTokenStream(lexer);
		RegexGrammarParser parser = new RegexGrammarParser(tokens);
		ParseTree tree = parser.start(); // begin parsing at rule start

		if (printRegex) {
			// Here, it is helpful to write some code which prints the regex after parsing.
			ParseTreeWalker walker = new ParseTreeWalker();
			RegexGrammarPrinter printer = new RegexGrammarPrinter();
		...
		ThompsonAutomatonCreator tca = new ThompsonAutomatonCreator();
		ParseTreeWalker walker = new ParseTreeWalker();
		walker.walk(tca, tree);
		Automaton aut = tca.obtainResult();
		System.out.println(aut.dotFormat());
